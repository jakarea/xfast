...
trait MailConfig
{
	...
	private function updateMailConfig(?array $settings = [], ?string $appName = null): void
	{
		...
		// MailerSend
		if (config('mail.default') == 'mailersend') {
			config()->set('services.mailersend.api_key', env('MAILERSEND_API_KEY', $settings['mailersend_api_key'] ?? null));
			config()->set('mail.from.address', env('MAIL_FROM_ADDRESS', $settings['mailersend_email_sender'] ?? ($mailSender ?? null)));
		}
	}
}
