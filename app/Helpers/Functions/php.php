 ...
 * @return string
 */
function prevent_str_containing_only_numeric_chars(?string $string): string
{
	return !is_numeric_strict($string) ? $string : '';
}
 ...
 * @return bool
 */
function is_numeric_strict(?string $string, bool $withRegex = true): bool
{
...
/**
 * Convert PHP array to JS pure array
 * @author: Anonymous
 *
 * Example usage:
 * $phpArray = array("apple", "banana", array("nested1", "nested2"), 42, true);
 * echo phpArrayToJsArray($phpArray); // Output: ["apple", "banana", ["nested1", "nested2"], 42, true]
 *
 * @param $phpArray
 * @return string
 */
function phpArrayToJsArray($phpArray): string
{
	$jsArray = "[";
	foreach ($phpArray as $element) {
		if (is_array($element)) {
			$jsArray .= phpArrayToJsArray($element);
		} elseif (is_string($element)) {
			$jsArray .= '"' . addslashes($element) . '"';
		} elseif (is_bool($element)) {
			$jsArray .= $element ? 'true' : 'false';
		} else {
			$jsArray .= $element;
		}
		$jsArray .= ', ';
