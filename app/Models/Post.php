...
class Post extends BaseModel implements Feedable
{
	...
	*/
	...
	{
		return Attribute::make(
			get: fn () => $this->getMainPictureUrl('picture-sm'),
		);
	}
	...
	{
		return Attribute::make(
			get: fn () => $this->getMainPictureUrl('picture-md'),
		);
	}
	...
	{
		return Attribute::make(
			get: fn () => $this->getMainPictureUrl('picture-lg'),
		);
	}
	...
	*/
	private function getMainPictureUrl(?string $size = null): ?string
	{
		...
		try {
			$picture = (isset($this->pictures) && $this->pictures->count() > 0)
				? $this->pictures->get(0)
				: null;
			
			$url = null;
			if (!empty($picture)) {
				$sizeLabel = config('larapen.media.resize.namedOptions.' . $size . '.label');
				$sizeLabel = !empty($sizeLabel) ? '_' . $sizeLabel : '';
				
				$urlColumn = 'filename_url' . $sizeLabel;
				$url = $picture->{$urlColumn};
	...
	{
		$defaultImg = config('larapen.media.picture');
