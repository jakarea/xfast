			...
			<div class="navbar-identity p-sm-0">
				...
				<a href="{{ url('/') }}" class="navbar-brand logo logo-title">
					<img src="{{ config('settings.app.logo_dark_url') }}"
						alt="{{ strtolower(config('settings.app.name')) }}" class="main-logo light-logo"
						data-bs-placement="bottom" data-bs-toggle="tooltip" title="{!! $logoLabel !!}" />
					<img src="{{ config('settings.app.logo_light_url') }}"
						alt="{{ strtolower(config('settings.app.name')) }}" class="main-logo dark-logo"
						data-bs-placement="bottom" data-bs-toggle="tooltip" title="{!! $logoLabel !!}" />
				</a>
				{{-- Toggle Nav (Mobile) --}}
			...
			<div class="navbar-collapse collapse" id="navbarsDefault">
				...
				<ul class="nav navbar-nav ms-auto navbar-right">
						...
						<ul id="authDropdownMenu" class="dropdown-menu user-menu shadow-sm">
							<li class="dropdown-item">
								...
								<a href="#quickLogin" class="nav-link" data-bs-toggle="modal"><i
										class="fa-solid fa-user"></i> {{ t('log_in') }}</a>
								@else
							...
							<li class="dropdown-item">
								<a href="{{ \App\Helpers\UrlGen::register() }}" class="nav-link"><i
										class="fa-regular fa-user"></i> {{ t('sign_up') }}</a>
							</li>
						</ul>
					</li>
					<li class="nav-item d-md-none d-sm-block d-block">
						@if (config('settings.security.login_open_in_modal'))
						<a href="#quickLogin" class="nav-link" data-bs-toggle="modal"><i class="fa-solid fa-user"></i>
							{{ t('log_in') }}</a>
						@else
						<a href="{{ \App\Helpers\UrlGen::login() }}" class="nav-link"><i class="fa-solid fa-user"></i>
							{{ t('log_in') }}</a>
						@endif
					</li>
					<li class="nav-item d-md-none d-sm-block d-block">
					...
					@if (auth()->user() && auth()->user()->hasRole('owner-staff')) 
						@if ((optional(auth()->user()->numberOfAdd)->value) > (auth()->user()->numberOfListing())) 
