						...
						<div class="row">
							<div class="col-8">
								<div class="welcome-msg">
									<h3 class="page-sub-header2 clearfix no-padding">{{ $authUser->name }} ! </h3>
									<span class="page-sub-header-sub small">
		                                {{ t('Last logged in at') }}: {{ \App\Helpers\Date::format($authUser->last_login_at, 'datetime') }}
		                            </span>
								</div>
							</div>
								...
								<div class="panel-collapse collapse {{ $userPanelClass }}" id="userPanel">
									<div class="card-body">
										<form name="details"
										      ...
										      role="form"
										      method="POST"
										      action="{{ route('update.staff') }}"
										      enctype="multipart/form-data"
										>
											{!! csrf_field() !!} 
											<input name="panel" type="hidden" value="user">
											<input name="staff_id" type="hidden" value="{{ $authUser->id}}">
