                                    ...
                                    <fieldset>
                                        ...
                                        @if (config('settings.single.show_listing_type'))
                                            ...
                                            <div id="postTypeBloc" class="row mb-3 required">
                                                <label class="col-md-3 col-form-label{{ $postTypeIdError }}">{{ t('type') }}
                                                    <sup>*</sup></label>
                                                <div class="col-md-8">
                                                    ...
                                                    @if (!empty($videos))
                                                        @for($i = 0; $i <= $videoLimit-1; $i++)
                                                            @php
                                                                ...
                                                                $vidId = data_get($videos, $i . '.id', $i);
                                                                $vidUrl = data_get($videos, $i . '.url.full', '');
                                                                try {
                                                                    $filePath = data_get($videos, $i . '.filename','');
                                                                    $fileExists = (isset($disk) && !empty($filePath) && $disk->exists($filePath));
                                                                    $videoSize = $fileExists ? (int)$disk->size($filePath) : 0;
                                                                } catch (\Throwable $e) {
                                                                    $videoSize = 0;
                                                                }
                                                            @endphp
                                                            <div class="mb-2{{ $videoError }}">
                                                                <div class="file-loading">
                                                                            ...
                                                                            data-initial-preview="{{ $vidUrl }}"
                                                                            data-initial-preview-as-data="true"
                                                                            data-initial-preview-file-type="video"
                                                                            data-initial-preview-config='[{
                                                                            "caption": "{{ basename($vidUrl) }}",
                                                                            "type": "video","size": "{{$videoSize}}",
                                                                            "filetype":"video/mp4",
                                                                            "url": "{{ url('posts/' . $post['id'] . '/photos/' . $vidId . '/delete/video11') }}",
                                                                             "key": "{{ $i }}"}]'
                                                                            data-show-cancel="false"
                                                                            data-show-upload="false"
                                                                            data-show-remove="false"
                                                                            data-show-browse="true"
                                                                            data-min-file-size="1"
                                                                            data-max-file-size="{{ $business == 1 ? 'Infinity' : $videoSizeLimit }}"
                                                                            data-file-action-settings='{
                                                                     "showRotate": false,
                                                                     "showUpload": false,
                                                                     "showDrag": false,
                                                                     "showRemove": true,
                                                                     ...
                                                                     "zoomClass": "btn btn-outline-secondary btn-sm"
                                                                            ...
                                                                            data-msg-size-too-large="The video file '{name}' ({size} KB) exceeds the maximum allowed file size of {maxSize}."
                                                                     >
