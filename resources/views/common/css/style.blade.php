...
@if (
	...
	|| !empty(config('settings.style.body_background_image'))
)
	body {
...
@if (!empty(config('settings.list.grid_view_cols')))
	...
	@if (config('lang.direction') == 'rtl')
		...
		html:not([theme="dark"]) .posts-wrapper.make-grid .item-list:nth-child(3n+3),
		html:not([theme="dark"]) .category-list.make-grid .item-list:nth-child(3n+3) {
			border-left: solid 1px #ddd;
		}
		...
		@media (max-width: 991px) {
			html:not([theme="dark"]) .posts-wrapper.make-grid .item-list:nth-child({{ $gridViewCols }}n+{{ $gridViewCols }}),
